...
function beamsIntersect(a1, a2, b1, b2) {
  ...
  if (Math.round(a1.x * 10000) === Math.round(a2.x * 10000)) return new Point(a1.x, slopeB * a1.x + (b1.y - slopeB * b1.x)); // Check for vertical line B
  else if (Math.round(b1.x * 10000) === Math.round(b2.x * 10000)) return new Point(b1.x, slopeA * b1.x + (a1.y - slopeA * a1.x));else {
...
Part.prototype.shorthand = function () {
  ...
  if (this.context.settings.debug) {
    ...
    shorthand.options = new Proxy(this.context.settings.options || {}, optionsProxy); // Proxy the absoluteOptions object

    const absoluteOptionsProxy = {
      get: function (absoluteOptions, name) {
        if (typeof absoluteOptions[name] === 'undefined') self.context.raise.warning(`Tried to access \`absoluteOptions.${name}\` but it is \`undefined\``);
        return Reflect.get(...arguments);
      },
...
function snappedOption(option, pattern) {
  ...
  const abs = conf.toAbs(pattern.settings.options[option], pattern.settings); // Handle units-specific config

  if (!Array.isArray(conf.snap) && conf.snap.metric && conf.snap.imperial) conf.snap = conf.snap[pattern.settings.units]; // Simple steps

  if (typeof conf.snap === 'number') return Math.ceil(abs / conf.snap) * conf.snap; // List of snaps

  if (Array.isArray(conf.snap) && conf.snap.length > 1) {
    for (const snap of conf.snap.sort((a, b) => a - b).map((snap, i) => {
      const margin = i < conf.snap.length - 1 ? (conf.snap[Number(i) + 1] - snap) / 2 // Look forward
...
Pattern.prototype.draft = function () {
  ...
  for (let i in this.settings.options) {
    if (typeof this.config.options[i] !== 'undefined' && typeof this.config.options[i].snap !== 'undefined' && this.config.options[i].toAbs instanceof Function) {
